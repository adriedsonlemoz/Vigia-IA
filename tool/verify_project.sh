...
grep -q '^version: 1\.0\.65+65$' pubspec.yaml || fail 'Versao esperada 1.0.65+65 nao encontrada.'
...
grep -q "static const String version = '1.0.65';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.65.'
grep -q 'static const int build = 65;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 65.'
grep -q "version: '1.0.65'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.65.'
...
grep -q '^version: 1.0.65+65$' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.65+65.'
...
grep -q '"version": "1.0.65"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.65.'
grep -q '"build": 65' app_identity.json \
  || fail 'app_identity.json nao esta no build 65.'
...
grep -q 'Vigia IA 1.0.65+65' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.65+65.'
