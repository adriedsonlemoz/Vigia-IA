...
grep -q '^version: 1\.0\.86+86$' pubspec.yaml || fail 'Versao esperada 1.0.86+86 nao encontrada.'
...
grep -q "static const String version = '1.0.86';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.86.'
grep -q 'static const int build = 86;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 86.'
grep -q "version: '1.0.86'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.86.'
...
grep -q '^version: 1.0.86+86$' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.86+86.'
...
grep -q '"version": "1.0.86"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.86.'
grep -q '"build": 86' app_identity.json \
  || fail 'app_identity.json nao esta no build 86.'
...
grep -q 'Vigia IA 1.0.86+86' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.86+86.'
...
# Mapa e GPS - 1.0.86
grep -q '^## 1.0.86+86' CHANGELOG.md || fail 'CHANGELOG nao documenta 1.0.86.'
grep -q 'Evolução 1.0.86' README.md || fail 'README nao documenta 1.0.86.'
grep -q "version: '1.0.86'" lib/screens/app_info_screen_components.dart \
  || fail 'Tela de Mudancas nao documenta 1.0.86.'
[[ -f RELEASE-1.0.86.md ]] || fail 'Notas da entrega 1.0.86 ausentes.'
