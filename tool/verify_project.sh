...
grep -q '^version: 1\.0\.87+87$' pubspec.yaml || fail 'Versao esperada 1.0.87+87 nao encontrada.'
...
grep -q "static const String version = '1.0.87';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.87.'
grep -q 'static const int build = 87;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 87.'
grep -q "version: '1.0.87'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.87.'
...
grep -q '^version: 1.0.87+87$' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.87+87.'
...
grep -q '"version": "1.0.87"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.87.'
grep -q '"build": 87' app_identity.json \
  || fail 'app_identity.json nao esta no build 87.'
...
grep -q 'Vigia IA 1.0.87+87' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.87+87.'
...
# Mapa e GPS - 1.0.86
grep -q '^## 1.0.87+87' CHANGELOG.md || fail 'CHANGELOG nao documenta 1.0.87.'
grep -q 'Evolução 1.0.87' README.md || fail 'README nao documenta 1.0.87.'
grep -q "version: '1.0.87'" lib/screens/app_info_screen_components.dart \
  || fail 'Tela de Mudancas nao documenta 1.0.87.'
[[ -f RELEASE-1.0.87.md ]] || fail 'Notas da entrega 1.0.87 ausentes.'
