...
grep -Fxq 'version: 1.0.121+121' pubspec.yaml || fail 'Versao esperada 1.0.121+121 nao encontrada.'
...
grep -Fq '## 1.0.121+121' CHANGELOG.md || fail 'CHANGELOG nao documenta 1.0.121.'
grep -q 'Evolução 1.0.121' README.md || fail 'README nao documenta 1.0.121.'
[[ -f RELEASE-1.0.121.md ]] || fail 'Notas da entrega 1.0.121 ausentes.'
...
grep -q "static const String version = '1.0.121';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.121.'
grep -q 'static const int build = 121;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 121.'
grep -q "version: '1.0.121'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.121.'
...
grep -Fq 'version: 1.0.121+121' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.121+121.'
...
grep -q '"version": "1.0.121"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.121.'
grep -q '"build": 121' app_identity.json \
  || fail 'app_identity.json nao esta no build 121.'
...
grep -Fq 'Vigia IA 1.0.121+121' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.121+121.'
...
grep -q 'VigiaIA/1.0.121' lib/services/route_explorer_service.dart || fail 'User-Agent do RouteExplorer nao acompanha a versao atual.'
...
grep -q "label: const Text('Baixar pontos')" lib/screens/monitor_screen_map_explorer.dart || fail 'Acao offline Baixar pontos ausente.'
