...
grep -q '^version: 1\.0\.97+97$' pubspec.yaml || fail 'Versao esperada 1.0.97+97 nao encontrada.'
...
grep -q "static const String version = '1.0.97';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.97.'
grep -q 'static const int build = 97;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 97.'
grep -q "version: '1.0.97'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.97.'
...
grep -q '^version: 1.0.97+97$' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.97+97.'
...
grep -q '"version": "1.0.97"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.97.'
grep -q '"build": 97' app_identity.json \
  || fail 'app_identity.json nao esta no build 97.'
...
grep -q 'Vigia IA 1.0.97+97' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.97+97.'
...
grep -q "expect(AppMetadata.version, '1.0.97');" test/app_metadata_test.dart \
  || fail 'Teste de AppMetadata nao esta sincronizado com 1.0.97.'
grep -q 'expect(AppMetadata.build, 97);' test/app_metadata_test.dart \
  || fail 'Teste de AppMetadata nao esta sincronizado com build 97.'
