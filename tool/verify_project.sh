...
grep -q '^version: 1\.0\.62+62$' pubspec.yaml || fail 'Versao esperada 1.0.62+62 nao encontrada.'
...
grep -q "static const String version = '1.0.62';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.62.'
grep -q 'static const int build = 62;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 62.'
grep -q "version: '1.0.62'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.62.'
...
grep -q '^version: 1.0.62+62$' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.62+62.'
...
grep -q '"version": "1.0.62"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.62.'
grep -q '"build": 62' app_identity.json \
  || fail 'app_identity.json nao esta no build 62.'
...
grep -q 'Vigia IA 1.0.62+62' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.62+62.'
...
# Buildfix do workflow Android - 1.0.62
grep -q '^## 1.0.62+62' CHANGELOG.md || fail 'CHANGELOG nao documenta 1.0.62.'
grep -q 'Evolução 1.0.62' README.md || fail 'README nao documenta 1.0.62.'
grep -q 'Evolução 1.0.62' ARCHITECTURE.md || fail 'ARCHITECTURE nao documenta 1.0.62.'
