...
grep -Fxq 'version: 1.0.110+110' pubspec.yaml || fail 'Versao esperada 1.0.110+110 nao encontrada.'
...
grep -q "static const String version = '1.0.110';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.110.'
grep -q 'static const int build = 110;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 110.'
grep -q "version: '1.0.110'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.110.'
...
grep -Fq 'version: 1.0.110+110' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.110+110.'
...
grep -q '"version": "1.0.110"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.110.'
grep -q '"build": 110' app_identity.json \
  || fail 'app_identity.json nao esta no build 110.'
...
grep -Fq 'Vigia IA 1.0.110+110' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.110+110.'
