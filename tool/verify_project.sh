...
grep -Fxq 'version: 1.0.127+127' pubspec.yaml || fail 'Versao esperada 1.0.127+127 nao encontrada.'
...
grep -Fq '## 1.0.127+127' CHANGELOG.md || fail 'CHANGELOG nao documenta 1.0.127.'
[[ -f RELEASE-1.0.127.md ]] || fail 'Notas da entrega 1.0.127 ausentes.'
...
grep -q "static const String version = '1.0.127';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.127.'
grep -q 'static const int build = 127;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 127.'
...
grep -Fq 'version: 1.0.127+127' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.127+127.'
...
grep -q '"version": "1.0.127"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.127.'
grep -q '"build": 127' app_identity.json \
  || fail 'app_identity.json nao esta no build 127.'
...
grep -Fq 'Vigia IA 1.0.127+127' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.127+127.'
...
grep -q 'VigiaIA/1.0.127' lib/services/route_explorer_service.dart || fail 'User-Agent do RouteExplorer nao acompanha a versao atual.'
