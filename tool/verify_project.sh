...
grep -q '^version: 1\.0\.85+85$' pubspec.yaml || fail 'Versao esperada 1.0.85+85 nao encontrada.'
...
grep -q "static const String version = '1.0.85';" lib/core/app_metadata.dart \
  || fail 'AppMetadata nao esta em 1.0.85.'
grep -q 'static const int build = 85;' lib/core/app_metadata.dart \
  || fail 'Build de AppMetadata nao esta em 85.'
grep -q "version: '1.0.85'" lib/screens/app_info_screen*.dart \
  || fail 'Tela Mudancas nao marca a versao 1.0.85.'
...
grep -q '^version: 1.0.85+85$' pubspec.yaml \
  || fail 'pubspec.yaml nao esta em 1.0.85+85.'
...
grep -q '"version": "1.0.85"' app_identity.json \
  || fail 'app_identity.json nao esta na versao 1.0.85.'
grep -q '"build": 85' app_identity.json \
...
grep -q 'Vigia IA 1.0.85+85' ARCHITECTURE.md || fail 'ARCHITECTURE nao esta em 1.0.85+85.'
...
# Telemetria compacta e diagnostico - 1.0.85
grep -q '^## 1.0.85+85' CHANGELOG.md || fail 'CHANGELOG nao documenta 1.0.85.'
grep -q 'Evolução 1.0.85' README.md || fail 'README nao documenta 1.0.85.'
grep -q "version: '1.0.85'" lib/screens/app_info_screen_components.dart \
  || fail 'Tela de Mudancas nao documenta 1.0.85.'
[[ -f RELEASE-1.0.85.md ]] || fail 'Notas da entrega 1.0.85 ausentes.'
